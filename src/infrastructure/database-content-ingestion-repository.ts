...
export class DatabaseContentIngestionRepository implements ContentIngestionRepository {
  ...
  public recordReceived(
    ...
    return updateContentIngestionWithEvent(
      this.database,
      record,
      "content_hash = ?, byte_length = ?",
      [contentHash, byteLength],
      "state = 'intake_initiated' AND content_hash IS NULL AND byte_length IS NULL",
      event,
    );
  }
  ...
  public markValidationPending(record: ContentIngestionRecord): Promise<void> {
    return updateContentIngestionState(
      this.database,
      record,
  ...
  public markAccepted(
    ...
    return updateContentIngestionWithEvent(
      this.database,
      record,
      "state = 'accepted', accepted_media_type = ?, accepted_at = ?",
      [acceptedMediaType, acceptedAt],
      "1 = 1",
      event,
    );
  }
  ...
  public markRejected(
    ...
    return updateContentIngestionWithEvent(
      this.database,
      record,
      "state = 'rejected', failure_code = ?, rejected_at = ?",
      [failureCode, rejectedAt],
      "1 = 1",
      event,
    );
  }

  public markProcessingFailed(
    record: ContentIngestionRecord,
    ...
    return updateContentIngestionWithEvent(
      this.database,
      record,
      "state = 'processing_failed', failure_code = ?, failed_at = ?",
