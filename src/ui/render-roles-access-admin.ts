...
<body>
  ...
  <main id="main" class="shell">
    ...
    <section class="panel" aria-labelledby="assign-title">
      <div class="section-heading"><div><p class="eyebrow">New assignment</p><h2 id="assign-title">Assign workspace role</h2></div><p>${activeMembers.length} active member${activeMembers.length === 1 ? "" : "s"} · ${assignableRoles.length} eligible role${assignableRoles.length === 1 ? "" : "s"}</p></div>
      <form method="post" action="/demo/app/admin/access/assign" class="assign-form">
        ...
        <label>Workspace role
          <select name="roleDefinitionId" required>
            ...
            ${assignableRoles.map((role) => `<option value="${escapeHtml(role.id)}">${escapeHtml(role.name)}${role.isSystem ? "" : " — custom"}</option>`).join("")}
          </select>
        </label>
