...
export function renderWorkspaceDocuments(
  ...
  return renderWorkspacePage(
    ...
    `<section class="intro" aria-labelledby="page-title">
      ...
      <p class="lede">Current state is calculated from persisted document/version evidence. An approval is shown as current only when it matches the exact current version and SHA-256 hash.</p>
    </section>${content}`,
  );
}
...
export function renderWorkspaceTemplates(
  ...
  return renderWorkspacePage(
    ...
    `<section class="intro" aria-labelledby="page-title">
      ...
      <p class="lede">Controlled templates preserve exact version, lifecycle, content hash, and provenance. Existing documents never silently inherit a later template change.</p>
    </section>${content}`,
  );
}
