...
<body>
  ...
  <main id="main" class="shell">
    ...
    <section class="intro" aria-labelledby="page-title">
      ...
      <p class="lede">Choose which immutable workflow versions are available in <strong>${escapeHtml(catalog.workspaceName)}</strong> and which exact version new workflow starts use by default.</p>
      ${notice ? `<p class="notice" role="status">${escapeHtml(notice)}</p>` : ""}
      <p><a href="/demo/app/admin/workflows">Manage tenant Workflow Definitions</a></p>
    </section>

    <section class="boundary" aria-labelledby="boundary-title">
      <p class="eyebrow">Selection boundary</p>
      <h2 id="boundary-title">Selection changes future starts, not history</h2>
      <ul>
        <li>Applicability is workspace-scoped and requires <code>workflow.manage</code>.</li>
        <li>Assignments always point to an exact immutable definition ID and version.</li>
        <li>Only an applicable version may be selected as the workspace default.</li>
        <li>The current default cannot be removed until another default is selected.</li>
...
function renderDefinition(
  ...
  const status = definition.isDefault
  ...
  const action = definition.applicable
    ? definition.isDefault
      ? '<p class="locked">Select another applicable version as default before removing this one.</p>'
      : `${actionForm(definition, "default", "Set as default")} ${actionForm(definition, "disable", "Remove from workspace", true)}`
    : actionForm(definition, "enable", "Make available");

  return `<article class="definition-card">
    <div class="definition-heading"><div><strong>${escapeHtml(definition.name)}</strong><span><code>${escapeHtml(definition.workflowDefinitionId)}</code> · v${definition.workflowDefinitionVersion}</span></div>${status}</div>
    <dl>
