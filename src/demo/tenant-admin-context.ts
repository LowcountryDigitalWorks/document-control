...
export interface GuidedTenantAdminContext {
  subjectId: string;
  membershipId: string;
  bindingId: string;
}
...
export function createGuidedTenantAdminContext(
  sessionId: string,
): GuidedTenantAdminContext {
...
export async function ensureGuidedTenantAdmin(
  database: DatabaseProvider,
  sessionId: string,
): Promise<GuidedTenantAdminContext> {
  const demo = createGuidedDemoContext(sessionId);
  const admin = createGuidedTenantAdminContext(sessionId);
