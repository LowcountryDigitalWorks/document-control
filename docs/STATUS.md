...
- Milestones include bootstrap PR #1, persisted-workflow PR #7, authorization PR #8, guided-workflow
  ...
  document retirement PR #33, linear template revision authoring PR #34, bounded workspace audit CSV export PR #35, versioned per-document evidence manifest export PR #36, queue-native review and approval actions PR #37, immutable document-version change summaries PR #38, and controlled template evidence detail PR #39.
...
### Production Readiness Foundation II — Operations & Supply-Chain

- Normal CI remains read-only at `permissions: contents: read`; all permanent `actions/checkout` steps use `persist-credentials: false`, no validation workflow pushes, and `pull_request_target` is not used.
- Permanent GitHub Actions are pinned to official full commit SHAs with human-readable version comments: `actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1` (`v7`), `pnpm/action-setup@0977fd99725f1db4007ccb2928dbb4e90d06cc86` (`v6`), and `actions/setup-node@820762786026740c76f36085b0efc47a31fe5020` (`v7`). Dependabot GitHub Actions updates remain enabled.
- `tests/unit/github-actions-security.test.ts` mechanically protects immutable Action pinning, checkout credential suppression, read-only token permissions, absence of `pull_request_target`, and continued Dependabot GitHub Actions maintenance.
- CodeQL was evaluated for the public TypeScript repository and deferred because the advanced result-upload workflow normally requires `security-events: write`; this foundation preserves the no-write PR validation boundary. A later CodeQL/default-setup or required-check decision needs separate governance review.
- `scripts/migration-files.ts` now owns deterministic repository migration discovery/application for tests and E2E setup. It requires a contiguous `NNNN_name.sql` sequence and rejects malformed, skipped, or reordered migration history.
- `tests/unit/migration-upgrade-path.test.ts` proves full clean creation through `0011` and the explicitly supported immediately-prior upgrade `0010 -> 0011` using real SQL. Representative identity/tenant/workspace/document-version/audit data survives, and cross-tenant references, append-only audit protection, required bounded change summaries, and summary immutability remain enforced after upgrade.
- Released migrations are documented as forward-only immutable history. Corrections require a new ordered migration; production execution requires pre-change state capture/backup, verified recovery prerequisites, ordered execution, post-migration schema/invariant checks, smoke checks, and recorded evidence. Destructive schema changes require their own reviewed recovery/rollback plan rather than automatic down-migration symmetry.
- `docs/OPERATIONS_RECOVERY.md` distinguishes portable application JSON, D1 metadata/state recovery, R2 binary/content recovery, and complete recoverable application state. The existing portable JSON export remains explicitly **not** a complete production backup.
- Recovery design accounts for the absence of one application-atomic D1/R2 transaction by requiring missing-content/orphan/hash/restore-point reconciliation, restoration sequencing, and post-restore verification of tenant/workspace relationships, version chains/hashes, provenance, workflow bindings, approvals, audit history, authorization state, configuration, and schema version.
- `tests/unit/recovery-drill.test.ts` performs deterministic local synthetic reconstruction into a fresh migrated SQLite database and verifies selected foreign-key, document/template hash, workflow, exact approval, role-binding, schema, and append-only audit invariants. The drill is not a production D1/R2 restore and establishes no RPO/RTO.
