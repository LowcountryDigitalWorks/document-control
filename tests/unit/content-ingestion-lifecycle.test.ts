...
describe("content ingestion lifecycle", () => {
  it("bounds untrusted metadata and never uses filename as storage identity", async () => {
    ...
    expect(normalizeDeclaredMediaType(" Application/PDF ")).toBe(
      "application/pdf",
    );
    for (const unsafe of [
      "../policy.pdf",
      ...
      ".",
      "..",
      `x${String.fromCharCode(0)}.pdf`,
    ]) {
      expect(() => normalizeDisplayFilename(unsafe)).toThrow(
        ContentIngestionInputError,
      );
    }
    expect(() =>
      normalizeDeclaredMediaType("application/pdf; charset=utf-8"),
    ).toThrow(ContentIngestionInputError);
    const { service } = fixture();
    const record = await initiate(service);
    expect(record.storageKey).toBe(
      "tenants/tenant-a/workspaces/workspace-a/content-ingestions/generated-1/staged-content",
    );
  ...
  it("enforces size and in-flight limits before storage", async () => {
    ...
    await expect(
      service.receiveAndValidate({
        ...
        bytes: new ArrayBuffer(CONTENT_INGESTION_MAX_BYTES + 1),
        occurredAt: at,
      }),
    ).rejects.toThrow(ContentIngestionInputError);
  ...
  it("persists SHA-256 and exposes only accepted, integrity-verified content", async () => {
    ...
    const accepted = await service.receiveAndValidate({
      tenantId: "tenant-a",
      workspaceId: "workspace-a",
      actorSubjectId: "subject-a",
      ingestionId: record.id,
    ...
    expect(store.objects.get(record.storageKey)?.contentType).toBe(
      "application/octet-stream",
    );
    const retrieved = await service.getAcceptedContent({
      tenantId: "tenant-a",
      workspaceId: "workspace-a",
      actorSubjectId: "subject-a",
      ingestionId: record.id,
    ...
    await expect(
      service.getAcceptedContent({
        tenantId: "tenant-a",
        workspaceId: "workspace-a",
        actorSubjectId: "subject-a",
        ingestionId: record.id,
      }),
    ).rejects.toThrow(ContentIngestionNotAvailableError);
  });

  it("keeps rejected and unresolved storage candidates unavailable", async () => {
    const { validator, service } = fixture();
    ...
    await expect(
      other.service.receiveAndValidate({
        tenantId: "tenant-a",
        workspaceId: "workspace-a",
        actorSubjectId: "subject-a",
        ingestionId: record.id,
    ...
    const failed = await other.service.recover({
      tenantId: "tenant-a",
      workspaceId: "workspace-a",
      actorSubjectId: "subject-a",
      ingestionId: record.id,
    ...
    expect(failed).toMatchObject({
      ...
      failureCode: "stored_content_missing",
    });
  });
});
