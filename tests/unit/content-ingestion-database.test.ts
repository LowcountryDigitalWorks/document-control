...
describe("content ingestion D1/SQLite boundary", () => {
  it("enforces tenant/workspace ownership in both lookup and database constraints", async () => {
    ...
    await expect(
      repository.find("tenant-b", "workspace-b", record.id),
    ).resolves.toBeNull();
    expect(() =>
      run(
        database,
  ...
  it("rechecks live membership before candidate bytes can be staged", async () => {
    ...
    run(
      database,
      "UPDATE tenant_memberships SET status = 'suspended' WHERE id = ?",
      "membership-a",
    );
    await expect(
    ...
    const stored = database
      .prepare(
        "SELECT content_hash AS hash, state FROM content_ingestions WHERE id = ?",
      )
      .get(record.id) as { hash: string | null; state: string };
  ...
  it("persists accepted integrity metadata and database-atomic minimized audit evidence", async () => {
    ...
    const events = database
      .prepare(
        "SELECT event_type AS type, payload_json AS payload FROM audit_events WHERE entity_type = 'content_ingestion' ORDER BY rowid",
      )
    ...
    expect(serialized).not.toMatch(
      /bearer|session|authorization[_-]?code|password|secret/i,
    );
    database.close();
  });
  ...
  it("blocks terminal rewrites, ownership relinking, event-cursor rewriting, and deletion", async () => {
    ...
    expect(() =>
      run(
        database,
        "UPDATE content_ingestions SET state = 'validation_pending' WHERE id = ?",
        record.id,
      ),
    ).toThrow(/lifecycle transition/i);
    expect(() =>
      run(
        database,
        "UPDATE content_ingestions SET storage_key = ? WHERE id = ?",
        "unsafe/relink",
        record.id,
      ),
    ).toThrow(/immutable/i);
    expect(() =>
      run(
        database,
        "UPDATE content_ingestions SET last_event_id = ? WHERE id = ?",
        "event-rewrite",
        record.id,
      ),
    ).toThrow(/event cursor/i);
    expect(() =>
      run(database, "DELETE FROM content_ingestions WHERE id = ?", record.id),
