...
test("filters documents with bounded literal server-side search", async ({
  page,
}) => {
  await createDocument(page);
  ...
  await expect(
    page.getByText("1 document matched.", { exact: false }),
  ).toBeVisible();
...
test("filters exact current approval state as the workflow changes", async ({
  page,
}) => {
...
test("filters templates by literal name and lifecycle", async ({ page }) => {
  ...
  await expect(
    page.getByText("1 template matched.", { exact: false }),
  ).toBeVisible();
...
test("rejects unsupported and oversized filters before querying", async ({
  page,
}) => {
  let response = await page.goto("/demo/app/documents?status=deleted");
  expect(response?.status()).toBe(400);
  await expect(
    page.getByText("Unknown document status.", { exact: true }),
  ).toBeVisible();

  response = await page.goto("/demo/app/templates?lifecycle=archived");
  expect(response?.status()).toBe(400);
  await expect(
    page.getByText("Unknown template lifecycle.", { exact: true }),
  ).toBeVisible();
